...
//region updater
  const url = config.updaterUrl + `/update/${process.platform}/${app.getVersion()}`;
  ...
  autoUpdater.on('update-downloaded', (event, releaseNotes, releaseName) => {
    const dialogOpts = {
      type: 'info',
      buttons: ['Thoát và cập nhật ngay', 'Đang chơi, để sau'],
      title: 'Cập nhật EasyGunny Launcher',
      message: process.platform === 'win32' ? releaseNotes : releaseName,
      detail: 'Đã có phiên bản mới, cập nhật ngay bây giờ?'
    }
